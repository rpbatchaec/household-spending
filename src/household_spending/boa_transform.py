...
CANON = [
    "Income", "Transfers", "Fees", "Taxes",
    "Housing", "Utilities", "Insurance",
    "Healthcare", "Transportation", "Fuel",
    "Groceries", "Dining", "Shopping",
    "Subscriptions", "Entertainment",
    "Travel", "Household", "Gifts/Charity",
    "Uncategorized",
]

ALIASES = {
    "atm fee": "Fees", "service fee": "Fees", "fee": "Fees",
    "mortgage": "Housing", "rent": "Housing", "hoa": "Housing",
    "electric": "Utilities", "water": "Utilities", "sewer": "Utilities",
    "gas bill": "Utilities", "internet": "Utilities", "phone": "Utilities",
    "insurance": "Insurance",
    "doctor": "Healthcare", "dental": "Healthcare", "pharmacy": "Healthcare",
    "medical": "Healthcare", "hospital": "Healthcare",
    "transportation": "Transportation", "tolls": "Transportation", "parking": "Transportation",
    "fuel": "Fuel", "gas": "Fuel",
    "grocery": "Groceries", "groceries": "Groceries",
    "restaurant": "Dining", "dining": "Dining", "coffee": "Dining",
    "shopping": "Shopping",
    "subscription": "Subscriptions", "subscriptions": "Subscriptions",
    "entertainment": "Entertainment",
    "travel": "Travel", "airfare": "Travel", "airline": "Travel", "hotel": "Travel",
    "household": "Household",
    "gift": "Gifts/Charity", "charity": "Gifts/Charity", "donation": "Gifts/Charity", "tithe": "Gifts/Charity",
    "transfer": "Transfers", "zelle": "Transfers", "online banking transfer": "Transfers",
    "salary": "Income", "payroll": "Income", "ssa": "Income", "deposit": "Income", "refund": "Income",
    "tax": "Taxes", "irs": "Taxes",
}

KEYWORD_RULES = [
    (["whole foods", "market basket", "stop & shop", "trader joe", "aldi", "shaw's"], "Groceries"),
    (["mcdonald", "starbucks", "dunkin", "chipotle", "pizza", "panera", "cafe", "restaurant"], "Dining"),
    ...
    (["marriott", "hilton", "airbnb", "delta", "american airlines", "united airlines", "jetblue"], "Travel"),
...
_VENDOR_STOPWORDS = {
    "authorization", "auth", "purchase", "pos", "debit", "credit", "recurring",
    "online", "banking", "transfer", "zelle", "visa", "mastercard", "mc",
    "card", "payment", "payment thank you", "pymt", "apple pay", "google pay",
    "ach", "echeck", "ck", "trnsfr", "fee", "adj", "refund"
}
...
def normalize_merchant(desc_clean: str) -> str:
    ...
    for keep in ["GEICO", "IRS", "MBTA", "BP", "AT&T"]:
        name = re.sub(fr"\b{keep.title()}\b", keep, name)
    return name
...
def _load_mapping(path: Path | None):
    ...
    exact = [(row.pattern, row.category) for _, row in m[m["type"]=="exact"].iterrows() if row.pattern]
    contains = [(row.pattern, row.category) for _, row in m[m["type"]=="contains"].iterrows() if row.pattern]
    regex = []
...
def transform(
    ...
    parsed_dates = pd.to_datetime(df[date_col].str.strip().replace("", pd.NA), errors="coerce", infer_datetime_format=True)
    ...
    if debit_col or credit_col:
        ...
        df["Amount"] = pd.to_numeric(credit, errors="coerce") - pd.to_numeric(debit, errors="coerce")
    else:
    ...
    if balance_col:
        df["Running Balance (num)"] = pd.to_numeric(df[balance_col].map(coerce_money), errors="coerce")
    ...
    for _, row in df.iterrows():
        ...
        if c:
            prov_counter[prov] += 1
            cats.append(_canonicalize(c)); continue
        ...
        if raw in ALIASES:
            ...
            cats.append(ALIASES[raw]); continue
        prov_counter["uncategorized"] += 1
        cats.append("Uncategorized")
    df["Category"] = cats

    front = [c for c in ["Account", "AccountSeq", "Date", "Amount", "Category", "Merchant"] if c in df.columns]
    ...
    if report:
        ...
        print("\n=== Transform Report ===")
        print(f"Rows: {total}")
        for key in ["mapping_exact","mapping_contains","mapping_regex","keyword_rule","alias_rule","alias_raw","uncategorized"]:
...
def learn_mappings(mapping_path: Path | None, learn_src: Path) -> tuple[int,int,int,int]:
    ...
    add_rows = []; dup = 0; conf = 0
    for _, r in learned.iterrows():
        key = r["key"]; cat = r["category"]
        if key in existing:
            if existing[key].lower() == cat.lower():
                ...
                conf += 1
            continue
        add_rows.append({"type": r["type"], "pattern": r["pattern"], "category": r["category"], "key": key})
    added = len(add_rows)
    if added:
        base = pd.concat([base, pd.DataFrame(add_rows)], ignore_index=True)
    base = base.drop(columns=["key"], errors="ignore").drop_duplicates(subset=["type","pattern"], keep="last")
    base = base.sort_values(["type", "pattern"]).reset_index(drop=True)
    base.to_csv(mapping_path, index=False)
    total = len(base)
    print(f"[learn] Added: {added}, duplicates: {dup}, conflicts (skipped): {conf}. Mapping size: {total}.")
    print(f"[learn] Mapping updated at: {mapping_path}")
    return (added, dup, conf, total)

def _build_argparser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Transform BOA CSV with vendor normalization and category mapping.")
    ...
    p.add_argument("--learn", action="store_true", help="Merge labeled rows into mapping BEFORE transform.")
    ...
    p.add_argument("--parquet", action="store_true", help="Also write a Parquet file (requires pyarrow or fastparquet).")
    return p
...
def transform_main(argv=None) -> int:
    ...
    learn_from = Path(args.learn_from) if args.learn_from else source.with_name(source.stem + "_uncategorized.csv")
    try:
        transform(
            source=source,
